  ...
  return (
    <div className='absolute top-0 right-0 left-0 bottom-0 z-10 backdrop-blur-sm bg-black/30 flex justify-center items-center'>
      ...
      <form className='relative bg-white p-10 rounded-xl text-slate-500'>
       ...
       {state !== "Login" && <div className='border px-6 py-2 flex items-center gap-2 rounded mt-5'>
            <img className='w-4 opacity-20' src={user_icon} alt="" />
            <input className='outline-none text-sm' type="text" placeholder='Full Name' required />
        </div>}

        <div className='border px-6 py-2 flex items-center gap-2 rounded mt-4'>
        ...
        <button type='submit' className='w-full bg-blue-600 text-white py-2 rounded'>{ state === "Login" ? "Login" : "Create account"}</button>
